...
@optional
/**
 * Called at the moment when a player joins a match that led to the player becoming a New Paying User (NPU) through this game
 */
...
///----------------------------------------------------
///@name Hybrid Overlay Methods
///----------------------------------------------------

/**
 * Initialize a hybrid session with OAuth grant JSON.
 *
 * This method initializes the Skillz SDK in hybrid mode, which allows Unity games to:
 * - Initialize the SDK with OAuth grant data without launching the full UI
 * - Show/hide the React Native UI overlay on demand
 * - Navigate to specific features without full launch
 * - Keep the React Native context warm in the background
 *
 * This method does NOT launch the Skillz UI. Use showHybridOverlay to display the UI after initialization.
 *
 * @param grantJSON OAuth grant JSON string containing access_token, username, user_id (can be nil for testing)
 * @param timestamp Timestamp of the grant (0 if not provided)
 * @param gameId Game ID string (required for React Native navigation - can be nil to use existing value)
 * @param autoLaunch If YES, automatically launches Skillz UI after initialization completes
 */
+ (void)initializeHybridSessionWithGrantJSON:(NSString * _Nullable)grantJSON
                                    timestamp:(long long)timestamp
                                       gameId:(NSString * _Nullable)gameId
                                   autoLaunch:(BOOL)autoLaunch;

/**
 * Shows the hybrid overlay (React Native UI) in hybrid mode.
 *
 * If the overlay is not already initialized, this will launch it.
 * The overlay remains initialized but hidden when not shown, allowing it to be shown again quickly.
 *
 * @param featureId Optional feature ID to navigate to a specific screen (e.g., "account_link")
 */
+ (void)showHybridOverlay:(NSString * _Nullable)featureId;

/**
 * Hides the hybrid overlay (React Native UI) in hybrid mode.
 *
 * The overlay remains initialized but hidden, allowing it to be shown again quickly.
 * The React Native context is kept alive for fast re-showing.
 */
+ (void)hideHybridOverlay;

/**
 * Navigates to a specific feature in the React Native UI based on feature ID.
 *
 * This is used in hybrid mode to show specific screens on demand.
 * The overlay must be shown first using showHybridOverlay.
 *
 * @param featureId The feature ID to navigate to (e.g., "account_link")
 */
+ (void)navigateToFeature:(NSString * _Nonnull)featureId;

/**
 * Navigates to a specific feature within the hybrid overlay with an optional offer ID.
 * The overlay must be shown first using showHybridOverlay.
 *
 * @param featureId The feature ID to navigate to (e.g., "checkout_deposit")
 * @param offerId Optional offer ID to use for checkout flows
 */
+ (void)navigateToFeature:(NSString * _Nonnull)featureId offerId:(NSString * _Nullable)offerId;
...
/**
 * Gets the Unity live fingerprint for iOS. This ensures Unity and native iOS use the exact same fingerprint.
 * Returns nil if Unity Framework is not found or on non-iOS platforms.
 *
 * @return Unity fingerprint string, or nil if unavailable
 */
+ (NSString * _Nullable)unityLiveFingerprint;

/**
 * Returns a bridge payload for analytics. Used by Unity and other integrations.
 * Type: 0–3. Returns nil on simulator or if unavailable. Caller does not own the returned string.
 *
 * @param type Payload type (0–3)
 * @return Payload string, or nil
 */
+ (NSString * _Nullable)getBridgePayloadForAnalyticsWithType:(int)type;
